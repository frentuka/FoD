program ej1;
    ...
    //     agregándole una opción para realizar bajas copiando el último registro del archivo
    //     en la posición del registro a borrar y luego truncando el archivo
...
begin
    ...
    writeln('');
    writeln(line_prefix, ' Números: Ingrese "-1" para no cambiar su valor');
    writeln(line_prefix, '   (Las limitaciones de Pascal no permiten');
    writeln(line_prefix, '    utilizar el mismo sistema que con el texto)');
    writeln('');
...
begin
    ...
    while not eof(emp_file) do begin
        read(emp_file, e);
        writeln(new_f, e.nombre, ' ', e.apellido, ' ', e.numero, ' ', e.dni, ' ', e.edad);
    end;
...
begin
    ...
    writeln(' (M): Modificar un empleado | (B): Buscar un empleado | (D): Borrar un empleado');
    writeln(' (X): Salir');
    readln(inp);
    ...
    while (inp <> 'X') and (inp <> 'x') do begin
        ...
        // Modificar el ejercicio 4 de la práctica 1 (programa de gestión de empleados),
        //     agregándole una opción para realizar bajas:
        //     copiando el último registro del archivo en la posición del registro a borrar
        //     y luego truncando el archivo en la posición del último registro de forma tal de evitar duplicados.

        if (inp = 'D') or (inp = 'd') then begin

            writeln;
            writeln('  Ingrese el número del empleado a eliminar');
            write('  -> '); readln(inp_id); writeln;

            // Volver al inicio del archivo
            seek(sel_file, 0);
            inp_emp.numero:= -32768; // Para evitar leer un dato no inicializado.
            while not eof(sel_file) and (inp_emp.numero <> inp_id) do
                read(sel_file, inp_emp);

            // re larga y re mal hecho:
            if inp_emp.numero = inp_id then begin
                inp_id:= filepos(sel_file) - 1; // Almacenar ubicación del empleado (debería usar otra variable)
                seek(sel_file, filesize(sel_file) - 1); // Mover puntero al último empleado del archivo
                read(sel_file, inp_emp); // Leer último empleado
                seek(sel_file, inp_id); // Mover puntero a la ubicación del empleado a eliminar
                write(sel_file, inp_emp); // Sobreescribir con el último empleado
                seek(sel_file, filesize(sel_file) - 1); // Mover a moverse al último empleado
                truncate(sel_file); // Acotar archivo, finalmente xd

                writeln;
                writeln(' Empleado eliminado correctamente.');
            end else begin
                writeln;
                writeln('El empleado #', inp_emp.numero, ' no existe.');
            end;
